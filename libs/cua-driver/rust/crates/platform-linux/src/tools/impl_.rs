...
fn cleanup_scoped_pointer_state(state: &ToolState, cursor_id: &str, reason: &str) {
    let recorded_backend = state
        ...
        .unwrap()
        .get(cursor_id)
        .map(|hold| hold.native_wayland);
    if recorded_backend != Some(false) {
        if let Err(error) = crate::wayland::persistent_vptr::forget_if_started(cursor_id) {
            tracing::warn!(
                cursor_id,
                reason,
                %error,
                "failed to release persistent Wayland pointer during scoped cleanup"
            );
        }
    }
    state.mouse_hold.lock().unwrap().remove(cursor_id);
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        match result {
            Ok(Ok(())) => {
                ...
                if session_ended {
                    if let Err(error) =
                        crate::wayland::persistent_vptr::forget_if_started(&cursor_id)
                    {
                        tracing::warn!(
                            cursor_id,
                            %error,
    ...
    if let Some(runtime_scope) = cua_driver_core::tool::current_dispatch_runtime_scope() {
        ...
        r.retain_runtime_cleanup(move || {
            let held_cursor_ids = state_for_runtime_cleanup
                ...
                .keys()
                .filter(|cursor_id| {
                    cursor_id.starts_with(&prefix) || cursor_id.as_str() == "default"
                })
                .cloned()
                .collect::<Vec<_>>();
...
mod persistent_hold_reconciliation_tests {
    use super::{reconcile_ended_persistent_hold, MouseHoldState, ToolState};

    #[test]
    fn canceled_persistent_gesture_clears_tool_hold_state() {
        let state = ToolState::new();
        state.mouse_hold.lock().unwrap().insert(
            "cursor-a".to_owned(),
            MouseHoldState {
                pid: 7,
                xid: 9,
