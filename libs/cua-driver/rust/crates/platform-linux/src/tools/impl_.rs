...
/// Return the cursor key only for a caller-owned, named session. Cursor
/// positioning for keyboard actions deliberately does not opt anonymous calls
/// (or the legacy cursor_id-only path) into session semantics.
fn named_session_cursor_key(args: &Value) -> Option<String> {
    args.get("session")
        .and_then(Value::as_str)
        .filter(|session| !session.is_empty())
        .map(runtime_scoped_default_cursor)
}
...
fn keyboard_window_center(xid: u64) -> Option<(f64, f64)> {
    ...
    if crate::wayland::is_wayland() {
        if crate::wayland::hyprland::is_session() {
            return crate::wayland::hyprland::window_center_pointer_target(xid)
                .ok()
...
fn current_pointer_position() -> Option<(f64, f64)> {
    if crate::wayland::is_wayland() {
        let point = if crate::wayland::hyprland::is_session() {
            crate::wayland::hyprland::cursor_position().ok()
        } else {
            crate::wayland::last_synth_cursor_pos()
        };
        return point.map(|(x, y)| (f64::from(x), f64::from(y)));
    }
...
fn explicit_keyboard_cursor_target(
    ...
    if let Some(element_index) = element_index {
        ...
        if crate::wayland::hyprland::is_session() {
            let target = crate::wayland::hyprland::global_pointer_target(sx, sy).ok()?;
            return Some((target.screen_x, target.screen_y));
        }
        return Some((sx, sy));
    }
    ...
    if crate::wayland::wayland_input_enabled() {
        return crate::wayland::try_window_local_to_output(xid, x.round() as i32, y.round() as i32)
            .ok()
            .map(|(sx, sy)| (f64::from(sx), f64::from(sy)));
    }
...
mod session_cursor_target_tests {
    ...
    #[test]
    fn only_explicit_named_sessions_opt_into_keyboard_cursor_positioning() {
        assert_eq!(
        ...
        assert_eq!(
            named_session_cursor_key(&json!({"_session_id": "implicit"})),
            None
        );
