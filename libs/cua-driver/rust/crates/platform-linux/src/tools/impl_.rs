    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        let result = tokio::task::spawn_blocking(move || -> anyhow::Result<_> {
            ...
            let (screen_w, screen_h) = if crate::wayland::is_wayland() {
                ...
                // Hyprland. The normalizer below converts the native buffer
                // into that exact action frame.
                crate::wayland::hyprland::logical_output_size_for_capture(native_w, native_h)
